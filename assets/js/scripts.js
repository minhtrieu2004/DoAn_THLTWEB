...
function loadCart() {
    fetch('../public/get_cart.php')
        ...
        .then(data => {
            ...
            if (data.cart.length === 0) {
                ...
                data.cart.forEach(item => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            <button class="btn btn-sm btn-secondary" onclick="updateCart('decrease','${item.name}')">-</button>
                            <span id="qty-${id}">${item.quantity}</span>
                            <button class="btn btn-sm btn-secondary" onclick="updateCart('increase','${item.name}')">+</button>
                        </td>
                        <td id="subtotal-${id}">${(item.price*item.quantity).toLocaleString()} đ</td>
                        <td>
                            <button class="btn btn-sm btn-danger" onclick="updateCart('remove','${item.name}')">Xóa</button>
                        </td>
                    `;
            ...
            if (subtotalEl) subtotalEl.textContent = data.totalPrice.toLocaleString() + ' đ';
            if (totalEl) totalEl.textContent = data.totalPrice.toLocaleString() + ' đ';
        });
}
